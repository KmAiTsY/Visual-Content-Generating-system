...
class VideoApp(QWidget):
    ...
    def on_button_click_1(self):
        ...
        # Генерація відео
        self.generated_video_path = self.return_video_with_filters(prompt, num_frames, num_steps, 
filter_type)
        import os
        video_path = os.path.abspath(self.generated_video_path)
 
        self.player1.setMedia(QMediaContent(QUrl.fromLocalFile(video_path))
)
        self.player1.play()
...
filter_type)
        ...
        self.player2.setMedia(QMediaContent(QUrl.fromLocalFile(video_path))
        )
        self.player2.play()
    ...
    def return_video_with_filters(self, prompt, num_frames, num_inference_steps, filter_type=None):
        ...
        if filter_type:
            video_frames = self.apply_filter(video_frames, filter_type)
